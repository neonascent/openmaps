...
// some reason extra 3px on this size
var CONFIG = {
	sizes: [
		{size: 256, postfix: ''}//,
...
ai.document = {
	...
	exportSlice: function(document, slice, file, size, opt_shouldUndo) {
        ...
        var currentPx = UnitValue(slice.width, "pt").as("px");
        
        
        var scale = (size * 100) / currentPx;
...
var main = function() {
	...
	util.array.forEach(slices, function(slice) {
       ...
       // ai.document.exportSlice(document, slice, file, 100, false);
        
		util.array.forEach(CONFIG.sizes, function(resolution) {
			var fileName = util.path.getFileName(relPath) + resolution.postfix;
			var file = new File(util.path.join([folder.absoluteURI, fileName]));

			ai.document.exportSlice(document, slice, file, resolution.size, false);
		});
	});
    
	// Batch undo changing artboardRect for a reason of performance issue
	var totalUndoes = slices.length * CONFIG.sizes.length;
	var i;
